...
class EmotionTagger:
    ...
    def get_emotions(self, text):
        ...
        try:
            ...
            processed_text = text.strip()[:512]  # Truncate for model limits
            
            if len(processed_text) < 3:  # Too short to analyze
                return default_emotions
            ...
            if isinstance(results, list) and len(results) > 0:
                ...
                # Now process the list of emotion results
            ...
            # If we got valid emotions, use them
            if emotion_scores and len(emotion_scores) > 0:
                ...
                for emotion, score in emotion_scores.items():
                    # Map any emotion names that might be different
    ...
    def analyze_political_emotions(self, df):
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Emotion analysis"):
            try:
                ...
                # Add default emotions for failed rows
                default_emotions = {
                    'anger': 0.1, 'disgust': 0.1, 'fear': 0.1, 
                    'joy': 0.1, 'neutral': 0.5, 'sadness': 0.1, 'surprise': 0.1,
                    'label': int(row['label']),
    ...
    def tag_dataset(self, input_file, output_file):
        ...
        # Load dataset info to understand what we're working with
        ...
        # Add dominant emotion safely
        try:
            emotion_cols = [f'emotion_{emotion}' for emotion in emotion_labels]
            
            # Ensure all emotion columns exist and have valid values
            for col in emotion_cols:
                if col not in df.columns:
                    df[col] = 0.1
                # Replace any NaN or invalid values
        ...
        # Analyze emotion patterns if political dataset
        ...
        try:
            ...
            if failed_tags > 0:
                print(f"⚠️  Failed tags: {failed_tags} (using defaults)")
